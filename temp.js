...
convo.ask((convo) => {
    const buttons = [
        { type: 'postback', title: 'Weblapkészítés', payload: 'USECASE_WEBPAGE' },
        //{ type: 'postback', title: 'Mobilalkalmazások fejlesztése', payload: 'USECASE_MOBILE' },
        //{ type: 'postback', title: 'Játékok készítése', payload: 'USECASE_GAME' },
        //{ type: 'postback', title: 'Önálló kis eszközök bütykölése (okos otthon, Raspberry Pi)', payload: 'USECASE_IOT' },
        { type: 'postback', title: 'Még nem tudom', payload: 'USECASE_UNKNOWN' }
    ];
    convo.sendButtonTemplate(`Mire szeretnéd használni?`, buttons);
}, (payload, convo, data) => {
    const text = payload.message.text;
    convo.set('usecase', text);
    convo.say(`Szóval ${text}`);
    convo.end();
}, [
    {
        event: 'postback:USECASE_WEBPAGE',
        callback: (payload, convo) => {
            endIntro(convo);
        }
    },
    {
        event: 'postback:USECASE_MOBILE',
        callback: (payload, convo) => {
            endIntro(convo);
        }
    },
    {
        event: 'postback:USECASE_GAME',
        callback: (payload, convo) => {
            endIntro(convo);
        }
    },
    {
        event: 'postback:USECASE_IOT',
        callback: (payload, convo) => {
            endIntro(convo);
        }
    },
    {
        event: 'postback:USECASE_UNKNOWN',
        callback: (payload, convo) => {
            endIntro(convo);
        }
    }
